...
class Agent(object):
    ...
    def save_params(self, model='base'):
        ...
        torch.save(self.__teacher.state_dict(), os.path.join(self.path_ckpt, '%s_t.pth' %(model)))
        torch.save(self.__student.state_dict(), os.path.join(self.path_ckpt, '%s_s.pth' %(model)))
